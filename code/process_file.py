...
package_file = st.file_uploader("Upload a text file of package data:", key="package_file")

# 1. Bytes to text. The upload is bytes; decode it, then split it into lines.
# TODO
if package_file:
    text = package_file.read().decode("utf-8").splitlines() if package_file else []
...
# TODO
if package_file:
    st.success(f"{len(parsed_packages)} packages written to data/{json_filename}")
